...
function readJson(req, maxBytes) {
  // Vercel may have already parsed the request stream.
  if (req.body !== undefined) {
    const text = Buffer.isBuffer(req.body) ? req.body.toString('utf8') : typeof req.body === 'string' ? req.body : JSON.stringify(req.body);
    if (Buffer.byteLength(text) > maxBytes) return Promise.reject(new ApiError(413, 'payload_too_large', 'Batas payload adalah 2 MB.'));
    try { return Promise.resolve(JSON.parse(text)); } catch { return Promise.reject(new ApiError(400, 'invalid_json', 'Body harus berupa JSON valid.')); }
  }
...
function createHandler(options = {}) {
  ...
  if (process.env.VERCEL && !databaseUrl && !options.store) throw Error('Vercel memerlukan DATABASE_URL PostgreSQL persisten.');
  const store = options.store ?? (databaseUrl ? new (require('./lib/postgres-store').PostgresStore)(databaseUrl) : new (require('./lib/store').Store)(options.databasePath ?? process.env.DATABASE_PATH ?? path.join(root, 'data', 'health-hub.sqlite')));
  const handler = async (req, res) => {
    ...
    try {
      if (url.pathname === '/api/health' && req.method === 'GET') {
        await store.ping(); send(res, 200, { status: 'ok', service: 'health-hub', api_version: 1 }); return;
      }
      if (url.pathname.startsWith('/api/')) {
        ...
        if (url.pathname === '/api/v1/records' && req.method === 'GET') { send(res, 200, await store.read(client.id, url.searchParams)); return; }
        if (url.pathname === '/api/v1/status' && req.method === 'GET') { send(res, 200, await store.status(client.id)); return; }
        if (url.pathname === '/api/v1/records' && req.method === 'POST') {
          ...
          send(res, 200, await store.receive(client.id, payload)); return;
        }
...
function createServer(options = {}) {
  ...
  server.on('close', () => { Promise.resolve(handler.close()).catch(() => {}); });
  return server;
}
...
module.exports = { createServer, createHandler, parseClients };
